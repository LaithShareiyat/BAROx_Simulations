...
def lap_time(track, v, v_eps=0.1):
    """
    Calculate lap time from velocity profile.

    Uses average velocity across each segment for accuracy:
        t = Σ(ds / v_avg)  where v_avg = (v[i] + v[i+1]) / 2
    ...
    """
    v_avg = 0.5 * (v[:-1] + v[1:])
    v_avg = np.maximum(v_avg, v_eps)
    return float(np.sum(track.ds / v_avg))
